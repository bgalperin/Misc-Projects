...
'	GOSUB gun_up  
	GOSUB gun_up_with_pan_tilt
	'WAIT
ELSEIF LASTCMD = &H72 THEN ' r  -  >  
'	GOSUB drop_to_knees  
	GOSUB gun_right_with_pan_tilt
	'WAIT
ELSEIF LASTCMD = &H62 THEN ' b  -  \/  
'	GOSUB gun_down  
	GOSUB gun_down_with_pan_tilt
	'WAIT
ELSEIF LASTCMD = &H6C THEN ' l  -  <  
'	GOSUB body_clap  
	GOSUB gun_left_with_pan_tilt
	'WAIT
...
'	GOSUB splap  
	WAIT
ELSEIF LASTCMD = &H65 THEN ' e  -  Select  
'	GOSUB   
	WAIT
ELSEIF LASTCMD = &H74 THEN ' t  -  Start  
	GOSUB standard_pose16 
	WAIT
ELSEIF LASTCMD = &H45 THEN ' E  -  o & Select  
'	GOSUB   
	WAIT
...
gun_up_with_pan_tilt:
	IF arm_up < 9 THEN
		arm_up = arm_up + 1
	ENDIF 
	GOSUB gun_move_with_pan_tilt
RETURN
gun_down_with_pan_tilt:
	IF arm_up > 1 THEN
		arm_up = arm_up - 1
	ENDIF 
	GOSUB gun_move_with_pan_tilt
RETURN
gun_left_with_pan_tilt:
	IF arm_left < 9 THEN
		arm_left = arm_left + 1
	ENDIF 
	GOSUB gun_move_with_pan_tilt
RETURN
gun_right_with_pan_tilt:
	IF arm_left > 1 THEN
		arm_left = arm_left - 1
	ENDIF 
	GOSUB gun_move_with_pan_tilt
...
gun_move_with_pan_tilt:
	...
	IF     arm_up=9 THEN
		MOVE G6B,,, 72,,,  '63
	ELSEIF arm_up=8 THEN
		MOVE G6B,,, 75,,,  '66
	ELSEIF arm_up=7 THEN
		MOVE G6B,,, 78,,,  '69
	ELSEIF arm_up=6 THEN 
		MOVE G6B,,, 82,,,  '72
	ELSEIF arm_up=5 THEN
		MOVE G6B,,, 84,,,  '75    
	ELSEIF arm_up=4 THEN 
		MOVE G6B,,, 87,,,  '78
	ELSEIF arm_up=3 THEN 
		MOVE G6B,,, 90,,,  '81
	ELSEIF arm_up=2 THEN
		MOVE G6B,,, 93,,,  '84
	ELSEIF arm_up=1 THEN
		MOVE G6B,,, 96,,,  '87
	ENDIF

	IF     arm_left=9 THEN
		MOVE G6B,,112,,,,  '117
	ELSEIF arm_left=8 THEN
		MOVE G6B,,108,,,,  '113
	ELSEIF arm_left=7 THEN
		MOVE G6B,,104,,,,  '109
	ELSEIF arm_left=6 THEN
		MOVE G6B,, 99,,,,  '104
	ELSEIF arm_left=5 THEN
		MOVE G6B,, 95,,,,  '100
	ELSEIF arm_left=4 THEN
		MOVE G6B,, 91,,,,  '96
	ELSEIF arm_left=3 THEN
		MOVE G6B,, 86,,,,  '91
	ELSEIF arm_left=2 THEN
		MOVE G6B,, 82,,,,  '87
	ELSEIF arm_left=1 THEN
		MOVE G6B,, 78,,,,  '83
	ENDIF
